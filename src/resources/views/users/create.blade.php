    ...
    <form method="POST" action="{{ route('users.store') }}">
        ...
        <div class="mb-4">
            ...
            <select name="access_level" class="w-full px-3 py-2 border rounded-lg focus:ring-indigo-500 focus:border-indigo-500" required>
                <option value="1" {{ old('access_level') == '1' ? 'selected' : '' }}>Admin (Level 1)</option>
                <option value="2" {{ old('access_level') == '2' ? 'selected' : '' }}>Editor (Level 2)</option>
                <option value="3" {{ old('access_level') == '3' ? 'selected' : '' }}>Staf (Level 3)</option>
                <option value="4" {{ old('access_level') == '4' ? 'selected' : '' }}>Attendee (Level 4)</option>
            </select>
        </div>
